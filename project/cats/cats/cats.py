...
def about(topic):
    ...
    def f(x):
        splitted = split(x)
        lst = [lower(remove_punctuation(word)) for word in splitted]
        for item in lst:
            if item in topic:
                return True
        return False
...
def wpm(typed, elapsed):
    ...
    "*** YOUR CODE HERE ***"
    len_words = len(typed)
    elasped_speed = elapsed / 60
    return len_words / 5 / elasped_speed
    # END PROBLEM 4
...
def shifty_shifts(start, goal, limit):
    ...
    def f(x, y, count, z):
        if len(x) == 0 or len(y) == 0:
            return abs(len(x) - len(y)) + count
        if count > z:
            return count
        elif x[0] == y[0]:
            return f(x[1:], y[1:], count,z)
        elif x[0] != y[0]:
            return f(x[1:], y[1:], count + 1, z)
...
def time_per_word(times_per_player, words):
    ...
    for item in times_per_player:
        lst += [[item[i] - item[i - 1] for i in range(1, len(item))]]
