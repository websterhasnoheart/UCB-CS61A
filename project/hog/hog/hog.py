...
def roll_dice(num_rolls, dice=six_sided):
    ...
    for i in range(0, num_rolls):
        score_this_turn = dice()
        if score_this_turn == 1:
            pig_out = 1
...
def free_bacon(score):
    ...
    # BEGIN PROBLEM 2
    "*** YOUR CODE HERE ***"
    trimmed_pi = pi // pow(10, 100-score)
    return trimmed_pi % 10 + 3
    # END PROBLEM 2
...
def swine_align(player_score, opponent_score):
    ...
    if (player_score == 0 or opponent_score == 0):
        return False
    while (i <= min(player_score, opponent_score)):
        if player_score % i == 0 and opponent_score % i == 0:
            common_divisor = i
        i += 1
        "终止条件位两个数字的最小值,比如30，60最大公约数为30，因此如果i比30大之后，就不可能找到更大的公约数了"
    ...
    while (score0 < goal and score1 < goal):
        if who == 0:
            ...
            while (extra_turn(score1, score0) and score1 < goal):
                ...
                say = say(score0, score1)
        who = other(who)
        #因为在判断了If为true之后else就不会执行了，所以在下面要切换回合
    # END PROBLEM 5
